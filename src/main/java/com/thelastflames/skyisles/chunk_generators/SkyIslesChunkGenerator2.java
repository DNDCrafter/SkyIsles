...
public class SkyIslesChunkGenerator2 extends ChunkGenerator {
    public static final Codec<SkyIslesChunkGenerator2> CODEC = RecordCodecBuilder.create((builder) -> {
        return builder.group(
                BiomeSource.CODEC.fieldOf("biome_source").forGetter((generator) -> generator.biomeSource),
                SkyIslesGeneratorSettings.DIRECT_CODEC.fieldOf("settings").forGetter((generator) -> generator.settings)
        ).apply(builder, builder.stable(SkyIslesChunkGenerator2::new));
    });

    private final SkyIslesGeneratorSettings settings;

    public SkyIslesChunkGenerator2(BiomeSource biomeSource, SkyIslesGeneratorSettings settings) {
        super(biomeSource);
        this.settings = settings;
    }

    @Override
    protected Codec<SkyIslesChunkGenerator2> codec() {
        return CODEC;
    }
    ...
    private ChunkAccess doFill(Blender pBlender, StructureManager pStructureManager, RandomState pRandom, ChunkAccess pChunk, int pMinCellY, int pCellCountY) {
        ...
        NoiseWrapper wrapper = settings.terrain().create(seed);
        NoiseWrapper noiseBottom = settings.bottom().create(seed);

        Cell center = new Cell(pChunk.getPos().x, pChunk.getPos().z);
        Cell[] cells = new Cell[]{
                Cell.of(center.x - 1, center.z + 1), Cell.of(center.x - 0, center.z + 1), Cell.of(center.x + 1, center.z + 1),
                Cell.of(center.x - 1, center.z - 0), Cell.of(center.x - 0, center.z - 0), Cell.of(center.x + 1, center.z - 0),
                Cell.of(center.x - 1, center.z - 1), Cell.of(center.x - 0, center.z - 1), Cell.of(center.x + 1, center.z - 1),
        };
        center = cells[4];
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                Cell[][] cells2d = new Cell[][]{
                        {cells[0], cells[1], cells[2]},
                        {cells[3], cells[4], cells[5]},
                        {cells[6], cells[7], cells[8]},
                };

                for (Cell[] row : cells2d) {
                    for (Cell cell : row) {
                        int wx = cell.modX(x + pChunk.getPos().getMinBlockX());
                        int wz = cell.modZ(z + pChunk.getPos().getMinBlockZ());

                        for (Cluster cluster : cell.clusters) {
                            double dist = cluster.sumDist(null, new Point(
                                    wx / 16d,
                                    wz / 16d
                            ));
                            ...
                            if (d > 0) {
                                int yStart = (int) cluster.yOffset;
                                ...
                                double nv = (wrapper.get((x + pChunk.getPos().getMinBlockX()) / 1000d, (z + pChunk.getPos().getMinBlockZ()) / 1000d) + 0.5);
                                if (nv < 0.25) nv = 0.25;
                                nv -= 0.25;
                                nv *= 2;
                                nv += 0.25;
                                ...
                                for (int y = 0; y < d * 20; y++) {
                                    pChunk.setBlockState(new BlockPos(x, yStart + y, z), Blocks.STONE.defaultBlockState(), false);
                                }
                                ...
                                nv = (noiseBottom.get(
                                        (x + pChunk.getPos().getMinBlockX()) / 10.0,
                                        (z + pChunk.getPos().getMinBlockZ()) / 10.0
                                ));
                                d += nv;

                                for (int y = 0; y < d * 30; y++) {
                                    pChunk.setBlockState(new BlockPos(x, yStart - y, z), Blocks.STONE.defaultBlockState(), false);
                                }
